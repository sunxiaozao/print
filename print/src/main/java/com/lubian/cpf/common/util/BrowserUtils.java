...
public class BrowserUtils {
	public static boolean isTrue(HttpServletRequest request){
		if((isIE(request)&&getIEversion(request)<11)||BrowserType.Chrome.equals(getBrowserType(request))){
			return true;
		}else{
			return false ;
		}
	...
	private static boolean getBrowserType(HttpServletRequest request, String brosertype) {
		return request.getHeader("USER-AGENT").toUpperCase().indexOf(brosertype) > 0 ? true : false;
	}
	...
	private final static String FIREFOX = "FIRWFOX";
