...
public class ZipUtil {
	 ...
	 * @throws Exception
	 */
	public static void zipFolder(String baseDir, String fileName)
			throws Exception {
	...
	public static void zipFile(String sourceFile, String fileName)
			throws Exception {
		ZipOutputStream zos = new ZipOutputStream(
				new FileOutputStream(fileName));
		zos.setEncoding("GBK");
	 ...
	 * @throws Exception
	 */
	public static List<String> unZipFile(String sourceFile, String destFolder)
			throws Exception {
		List<String> filenames=new ArrayList<>();
		File fileDelete = new File(destFolder);
		...
		while (zList.hasMoreElements()) {
			...
			File fl=getRealFileName(destFolder, ze.getName());
			filenames.add(fl.getPath());
	 ...
	 * @throws Exception
	 */
	public static void unZipFile(File sourceFile, String destFolder)
			throws Exception {
		File fileDelete = new File(destFolder);
		...
		while (zList.hasMoreElements()) {
			...
			OutputStream os = new BufferedOutputStream(new FileOutputStream(
					getRealFileName(destFolder, ze.getName())));
			InputStream is = new BufferedInputStream(zfile.getInputStream(ze));
			int readLen = 0;
			while ((readLen = is.read(buf, 0, 1024)) != -1) {
				os.write(buf, 0, readLen);
